...
bool Damage::isReceptive(const Mechanics * m, const battle::Unit * unit) const
{
	...
	bool isImmune = (unit->getBonusBearer()->valOfBonuses(BonusTypeID::SPELL_DAMAGE_REDUCTION, -1) >= 100); //General spell damage immunity
	//elemental immunity for damage
	m->getSpell()->forEachSchool([&](const SchoolInfo & cnf, bool & stop)
	{
		isImmune |= (unit->getBonusBearer()->valOfBonuses(BonusTypeID::SPELL_DAMAGE_REDUCTION, static_cast<ui8>(cnf.id)) >= 100); //100% reduction is immunity
	});
