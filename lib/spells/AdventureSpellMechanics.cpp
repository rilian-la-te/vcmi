...
{
	AdvmapSpellCast asc;
	asc.casterID = parameters.caster->getAdventureCasterID();
...
{
	return env->getCb()->getHero(parameters.caster->getAdventureCasterID());
}
...
ESpellCastResult SummonBoatMechanics::applyAdventureEffects(SpellCastEnvironment * env, const AdventureSpellCastParameters & parameters) const
{
	...
	else //create boat
	{
		NewObject no;
		no.ID = Obj::BOAT;
		no.subID = getHeroCaster(env, parameters)->getBoatType().getNum();
...
ESpellCastResult DimensionDoorMechanics::applyAdventureEffects(SpellCastEnvironment * env, const AdventureSpellCastParameters & parameters) const
{
	...
	if(getHeroCaster(env, parameters)->getBonuses(Selector::source(BonusSource::SPELL_EFFECT, owner->id), Selector::all, cachingStr.str())->size() >= owner->getLevelPower(schoolLevel)) //limit casts per turn
	{
		InfoWindow iw;
		iw.player = parameters.caster->getCasterOwner();
	...
	else if(env->moveHero(parameters.caster->getAdventureCasterID(), getHeroCaster(env, parameters)->convertFromVisitablePos(parameters.pos), true))
	{
		SetMovePoints smp;
		smp.hid = parameters.caster->getAdventureCasterID();
...
ESpellCastResult TownPortalMechanics::applyAdventureEffects(SpellCastEnvironment * env, const AdventureSpellCastParameters & parameters) const
{
	...
	if(env->moveHero(parameters.caster->getAdventureCasterID(), getHeroCaster(env, parameters)->convertFromVisitablePos(destination->visitablePos()), true))
	{
		SetMovePoints smp;
		smp.hid = parameters.caster->getAdventureCasterID();
		smp.val = moveCost;
