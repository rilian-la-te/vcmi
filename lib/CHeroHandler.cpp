...
CHeroClass::CHeroClass()
 : faction(0),  affinity(0), defaultTavernChance(0), commander(nullptr)
{
}
...
{
	std::vector<std::shared_ptr<Bonus>> result;
	for(const std::shared_ptr<Bonus>& oldBonus : spec.bonuses)
	{
		oldBonus->sid = sid;
...
void CHeroHandler::afterLoadFinalization()
{
	for(auto & hero : objects)
	{
		...
		if(!hero->specDeprecated.empty() || !hero->specialtyDeprecated.empty())
		{
			...
			{
				for(const std::shared_ptr<Bonus>& b : SpecialtyBonusToBonuses(spec, hero->ID.getNum()))
					convertedBonuses.push_back(b);
			}
