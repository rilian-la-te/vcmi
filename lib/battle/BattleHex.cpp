...
std::vector<BattleHex> BattleHex::neighbouringTiles() const
{
	...
	ret.reserve(6);
	for(EDir dir = static_cast<EDir>(0); dir <= static_cast<EDir>(5); dir = static_cast<EDir>(dir+1))
...
uint8_t BattleHex::getDistance(BattleHex hex1, BattleHex hex2)
{
	...
	int x1 = static_cast<int>(hex1.getX() + y1 * 0.5);
	int x2 = static_cast<int>(hex2.getX() + y2 * 0.5);
