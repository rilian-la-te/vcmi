...
public:
	...
	virtual std::vector<std::shared_ptr<const CObstacleInstance>> getAllAffectedObstaclesByStack(const battle::Unit * unit, const BattleHex & dest, const std::vector<BattleHex> & passed) const = 0;
};
