...
void MetaString::getLocalString(const std::pair<ui8,ui32> &txt, std::string &dst) 
{
	int type = txt.first;
	int ser = txt.second;
...
CGHeroInstance * CGameState::HeroesPool::pickHeroFor(bool native, const PlayerColor& player, const CTown *town,
...
CGameState::CampaignHeroReplacement::CampaignHeroReplacement(CGHeroInstance * hero, const ObjectInstanceID& heroPlaceholderId) : hero(hero), heroPlaceholderId(heroPlaceholderId)
{
...
struct statsHLP
{
	...
	static int getIncome(const PlayerState * ps)
	{
		...
		for(const CGObjectInstance * obj : CGObjectInstance::cb->gameState()->map->objects)
		{
...
std::map<ui32, ConstTransitivePtr<CGHeroInstance> > CGameState::unusedHeroesFromPool()
{
	...
	for (const auto & player : players)
		for(auto availableHeroe : player.second.availableHeroes)
			if(availableHeroe)
				pool.erase((*availableHeroe).subID);
