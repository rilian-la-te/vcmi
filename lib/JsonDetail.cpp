...
JsonNode JsonParser::parse(std::string fileName)
{
	...
	if (!errors.empty())
	{
		logMod->warn("File %s is not a valid JSON file!", std::move(fileName));
		logMod->warn(errors);
	}
	return root;
}
...
namespace
{
	namespace Common
	{
		std::string emptyCheck(Validation::ValidationData & /*unused*/, const JsonNode & /*unused*/, const JsonNode & /*unused*/, const JsonNode & /*unused*/)
		{
			// check is not needed - e.g. incorporated into another check
			return "";
		}

		std::string notImplementedCheck(Validation::ValidationData & /*unused*/, const JsonNode & /*unused*/, const JsonNode & /*unused*/, const JsonNode & /*unused*/)
		{
		...
		std::string schemaListCheck(Validation::ValidationData & validator, const JsonNode & baseSchema, const JsonNode & schema, const JsonNode & data,
									const std::string& errorMsg, const std::function<bool(size_t)>& isValid)
		{
	...
	namespace Struct
	{
		...
		{
			validator.currentPath.emplace_back();
			validator.currentPath.back().String() = std::move(nodeName);
			auto onExit = vstd::makeScopeGuard([&]()
			{
				validator.currentPath.pop_back();
			});
