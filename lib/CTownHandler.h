...
public:
	...
	static void addNewBonus(const std::shared_ptr<Bonus>& b, BonusList & bonusList);
...
class DLL_LINKAGE CTownHandler : public CHandlerBase<FactionID, Faction, CFaction, FactionService>
{
	...
	static void loadBuildingRequirements(CBuilding * building, const JsonNode & source, std::vector<BuildingRequirementsHelper> & bidsToLoad);
	void loadBuilding(CTown * town, const std::string & stringID, const JsonNode & source);
	void loadBuildings(CTown * town, const JsonNode & source);

	static std::shared_ptr<Bonus> createBonus(CBuilding * build, Bonus::BonusType type, int val, int subtype = -1);
	static std::shared_ptr<Bonus> createBonus(CBuilding * build, Bonus::BonusType type, int val, TPropagatorPtr & prop, int subtype = -1);
	static std::shared_ptr<Bonus> createBonusImpl(const BuildingID& building, Bonus::BonusType type, int val, TPropagatorPtr & prop, const std::string & description, int subtype = -1);

	/// loads CStructure's into town
	static void loadStructure(CTown &town, const std::string & stringID, const JsonNode & source);
	static void loadStructures(CTown &town, const JsonNode & source);

	/// loads town hall vector (hallSlots)
	static void loadTownHall(CTown &town, const JsonNode & source);
	static void loadSiegeScreen(CTown &town, const JsonNode & source);

	static void loadClientData(CTown &town, const JsonNode & source);

	void loadTown(CTown * town, const JsonNode & source);

	static void loadPuzzle(CFaction & faction, const JsonNode & source);
...
public:
	...
	static void addBonusesForVanilaBuilding(CBuilding * building);
